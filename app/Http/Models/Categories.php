...
class Categories extends Model
{
    ...
    protected $fillable = ['cat_name','par_cat_id','status'];
    ...
    public function get_category_select_list($catid = 0, $space=''){
        ...
        if($data){
            ...
            foreach($catrow as $row){
                $this->html .= "<option value='".$row['cat_id']."'>".$space.$row['cat_name']."</option>";
                $this->get_category_select_list($row['cat_id'],$space);
            }
        }
        return $this->html;
    }
    ...
    public function get_categories($catid = 0){
        ...
        if($data){
            ...
            foreach ($result as $row) {
                ...
                $this->html.= "<li dataid=".$row['cat_id'].">".$row['cat_name'];
                $this->get_categories($row['cat_id']);
                $this->html.= '</li>';
                $i++;
                if ($i > 0){
                    $this->html.= '</ul>';
    ...
    public function get_edit_row_data($id = 0){
        $Model = new static;
        $data = $Model->where('cat_id',$id)->first();
        if($data){
            return $data->toArray();
        }else{
            return false;
